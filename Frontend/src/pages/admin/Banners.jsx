  ...
  return (
    <RoleDashboardLayout role="admin" title="Homepage Banners">
      <div className="space-y-6">
        <form onSubmit={onCreate} className="bg-white rounded-xl shadow p-5 space-y-4">
          ...
          <div className="grid md:grid-cols-2 gap-3 items-start">
            ...
            <div className="border rounded-lg p-2 min-h-[130px] bg-gray-50">
              {form.image_url ? (
                <img src={form.image_url} alt="Banner preview" className="w-full h-28 object-cover rounded" />
              ) : (
                <p className="text-sm text-gray-500">Image preview appears here.</p>
              )}
            </div>
          </div>
        ...
        <div className="bg-white rounded-xl shadow p-5">
          ...
          {!loading && !sortedRows.length && <p className="text-sm text-gray-500">No banners found.</p>}
          <div className="space-y-4">
            {sortedRows.map((banner) => (
              <div key={banner.id} className="border rounded-lg p-3 grid md:grid-cols-4 gap-3 items-center">
                <img
